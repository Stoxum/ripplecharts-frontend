...
angular.module('stoxumcharts.landing', [
  'ui.state'
])
  ...
  .controller('LandingCtrl', function LandingCtrl($scope, $state, gateways) {
    ...
    $scope.metrics = {
      totalTradeVolume: {
        label: 'Total STM Trade Volume <small>(All Exchanges)</small>',
        link: '#/stm-markets'
      },
      tradeVolumeRCL: {
        label: 'STM Ledger Trade Volume <small>(All Pairs)<small>',
        link: '#/trade-volume'
      },
      paymentVolumeRCL: {
        label: 'STM Ledger Payment Volume'
      },
      capitalizationSTM: {
    ...
    function setValueRates(currency, useCached, callback) {
      ...
      function apply() {
        ...
        $scope.valueRatePair = 'STM/' + currency
        callback()
      }
    ...
    */
    ...
    function filterSTMVolume(components) {
      var total = 0
      var count = 0

      components.forEach(function (c) {
        if (c.base.currency === 'STM' ||
          c.counter.currency === 'STM') {
    ...
    function getMetricValues() {
      ...
      api.getExchangeVolume({}, function (err, resp) {
        ...
        stmVolume = filterSTMVolume(components)

        // add RCL STM volume to total metric
        if ($scope.metrics.totalTradeVolume.components &&
          !$scope.metrics.totalTradeVolume.withRCL) {
          $scope.metrics.totalTradeVolume.components.unshift(stmVolume)
          setMetricValue('totalTradeVolume',
            $scope.metrics.totalTradeVolume.total + stmVolume.base_volume)
      ...
      api.getExternalMarkets({}, function (err, resp) {
        ...
        if ($scope.metrics.tradeVolumeRCL.components) {
          stmVolume = filterSTMVolume($scope.metrics.tradeVolumeRCL.components)
          $scope.metrics.totalTradeVolume.components.unshift(stmVolume)
          total += stmVolume.base_volume
    ...
    $scope.$watch('totalSTM', function (d) {
      setMetricValue('capitalizationSTM', d)
    })
    ...
    /*
    api.getSTMstats(function(err, resp) {
      $scope.totalSTM = resp ? resp.total : undefined
      $scope.$apply()
    })
    */
