angular.module('stoxumcharts.manage-gateways', [
  'ui.state',
  'ui.bootstrap'
])
  ...
  .controller('ManageGatewaysCtrl', function ManageGatewaysCtrl($scope, $state, $location, gateways) {
    ...
    $scope.base = store.session.get('base') || store.get('base') ||
      Options.base || { currency: "STM", issuer: "" };
    ...
    //swap it if STM is the currency
    //this can happen if base/trade isnt
    //indicated in the URL
    if ($scope[query].currency === 'STM') {
    ...
    function add() {
      ...
      d3.xhr('https://id.stoxum.com/v1/user/' + newGateway, function (err, res) {
        if (!err) {
        ...
        if (newGateway.length >= 30 && newGateway.length <= 40 && newGateway[0] === "r") {
          ...
          description.html('Please enter a valid Stoxum Address.');
        }
      });
    }
    ...
    function pickNextCurrency(currency) {
      ...
      for (var i = 0; i < currencyList.length; i++) {
        c = currencyList[i];
        if (picked && c.include && c.currency !== "STM") {
          return c;
        }
