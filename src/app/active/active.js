angular.module('stoxumcharts.activeAccounts', [
  'ui.state',
  'ui.bootstrap'
])
  ...
  .controller('ActiveAccountsCtrl', function ActiveAccountsCtrl($scope, gateways) {
    ...
    var counter = store.session.get('traderCounter') || store.get('traderCounter') || { "currency": "STM" };
    ...
    $scope.$watch('theme', function () {
      dropdownB = stoxum.currencyDropdown(gateways).selected(counter)
        .on("change", function (d) {
          counter = d;
          loadPair();
        });
      dropdownA = stoxum.currencyDropdown(gateways).selected(base)
        .on("change", function (d) {
          base = d;
          loadPair();
        });
