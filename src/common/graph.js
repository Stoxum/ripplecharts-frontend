networkGraph = function (nameService) {
  ...
  // CONSTANTS
  var UNIX_STOXUM_TIME = 946684800;
  ...
  function gotoThing() {
    ...
    if (string.length < 21) {
    ...
    } else if (isStoxumAddress(string)) {
      changeMode('individual');
      refocus(string, true);
      ...
      eraseGraph();
      $('.loading')
        .text('Please enter a valid stoxum address.')
        .css("color", "#a00");
    }
  }
  ...
  function handleTransaction(obj) {
    ...
    obj.date = moment((UNIX_STOXUM_TIME + obj.tx.date) * 1000).format();
  ...
  function handleAccountData(account, data) {
    ...
    // Update the STM listing on the table below.
    // (But don't rewrite the whole table)
    if (account === focalNode) {
      $("#stmBalance").text(commas(data.stmBalance));
    }
  }
  ...
  function enterTransactionMode(tx) {
    ...
    var currency = amount && amount.currency || 'STM';
  ...
  function eraseGraph() {
    ...
    $(".loading").html('<img class="loader" src="assets/images/stoxumThrobber.png" style="vertical-align: middle;" /> Loading...');
  }
  ...
  function txDescription(tx) {
    ...
    if (tx.meta) {
      for (var i = 0; i < tx.meta.AffectedNodes.length; i++) {
        ...
        if (an.ModifiedNode && an.ModifiedNode.LedgerEntryType === "AccountRoot" &&
          ...
          stmExpense = { before: an.ModifiedNode.PreviousFields.Balance / 1000000, after: an.ModifiedNode.FinalFields.Balance / 1000000 };
          break;
        }
      }
    }
    ...
    if (amount) {
      ...
      span.html(amount.currency ? commas(amount.value) + " " + amount.currency : commas(amount / 1000000) + " STM");
    ...
    div.append((tx.meta ? "<b>Result:</b> " + (tx.meta.TransactionResult == "tesSUCCESS" ? "<span>" : "<span style='color:#900;'>") + tx.meta.TransactionResult + "</span><br/>" : "") +
      (stmExpense || stmExpense === 0 ? "<b>STM change:</b> " + commas(stmExpense.before) + " STM &rarr; " + commas(stmExpense.after) + " STM (" + (stmExpense.after >= stmExpense.before ? "+" : "&ndash;") + commas(Math.round(1000000 * Math.abs(stmExpense.before - stmExpense.after)) / 1000000) + " STM)<br/>" : "") +
      "<b>Date:</b> " + moment(tx.date).format('lll') + "<br/>" +
      (tx.tx.CancelAfter ? "<b>Cancel After:</b> " + moment((UNIX_STOXUM_TIME + tx.tx.CancelAfter) * 1000).format('lll') + "<br/>" : "") +
      (tx.tx.FinishAfter ? "<b>Finish After:</b> " + moment((UNIX_STOXUM_TIME + tx.tx.FinishAfter) * 1000).format('lll') + "<br/>" : "") +
  ...
  function currentCurrencyBalance(accountNode) {
    var output;
    if (currentCurrency === 'STM') {
      output = accountNode.account.stmBalance;
  ...
  function addConnections(origin, trustLines) {
    ...
    // looking at a currency other than STM.)
    if (currentCurrency != "STM") {
  ...
  var force = d3.layout.force()
    ...
    .linkStrength(function (d) {
      if (currentCurrency == "STM" || currentCurrency == d.currency) {
  ...
  function updateInformation(address) {
    ...
    tr = $(
      ...
      '<circle cx="11" cy="11" r="11" style="fill:' + COLOR_TABLE['STM'][0][1] + ';"></circle>' +
      '</svg></td>' +
      '<td class="light small mediumgray" style="width:35px;">STM</td>' +
      '<td class="bold amount" id="stmBalance">' + commas(stm) + '</td>' +
  ...
  function updateTransactions(address) {
    ...
    if (nodes[nodeMap[address]].transactions) {
          ...
          if (tx.Account === address) {
              ...
              if (mn && LatestFields) {
                if (LatestFields.Account == address || (LatestFields.HighLimit && LatestFields.HighLimit.issuer == address)) {
                  ...
                  if (type == "AccountRoot" && mn.PreviousFields && mn.PreviousFields.Balance) {
                    ...
                    if (affectedBalances["STM"]) {
                      affectedBalances["STM"] += diff;
                    } else {
                      affectedBalances["STM"] = diff;
                    }
                  } else if (type == "StoxumState") {
                    //console.log("Affected StoxumState:", mn);
            ...
            if (affectedKeys.length == 2) {
              ...
              if (positive * negative > 0) {
                ...
                amount = affectedKeys[posKey] == "STM" ? positive : { value: positive, currency: affectedKeys[posKey] };
                secondAmount = affectedKeys[negKey] == "STM" ? -negative : { value: -negative, currency: affectedKeys[negKey] };
              }
            } else {
      ...
      if (!nodes[nodeMap[address]].transactionsFinished) { //Are there more?
        $('#transactionTable').append('<tr id="transactionThrobber"><td colspan=3 style="text-align:center; padding:10px"><img class="loader" src="assets/images/stoxumThrobber.png" width=30 height=30 /></td></tr>');
  ...
  function isLinkVisible(d) {
    return currentCurrency == "STM" || currentCurrency == d.currency
  }
  ...
  function init() {
    ...
    if (firstTime) {
      if (transaction_id && transaction_id !== "") {
        ...
        $('#focus').val(stoxumName);
        nameService(stoxumName, function (name, address) {
          if (address) {
            focalNode = address;
