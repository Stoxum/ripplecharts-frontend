...
angular.module('txsplain', [])
  .directive('txsplain', ['$http', 'stoxumName', function ($http, stoxumName) {
    ...
    var txFlags = {
      ...
      AccountSet: {
        ...
        0x00100000: 'tfDisallowSTM',
        0x00200000: 'tfAllowSTM'
      },
      OfferCreate: {
      ...
      TrustSet: {
        ...
        0x00020000: 'tfSetNoStoxum',
        0x00040000: 'tfClearNoStoxum',
    ...
    function displayAmount(amount, isFee) {
      if (isFee) {
        return '<b>' + renderNumber(Number(amount) / 1000000) + '</b> STM'
    ...
    return {
      ...
      link: function (scope, element) {
        ...
        function renderMeta(tx) {
          ...
          // render StoxumSate
          function renderStoxumState(action, node) {
            var fields = node.FinalFields || node.NewFields
            var prev = node.PreviousFields
          ...
          function renderOfferChanges(node) {
            var html = ''
            var paysCurrency = node.FinalFields.TakerPays.currency || 'STM'
            var paysIssuer = node.FinalFields.TakerPays.issuer || ''
            var getsCurrency = node.FinalFields.TakerGets.currency || 'STM'
          ...
          function renderOfferNode(action, node) {
            var fields = node.FinalFields || node.NewFields
            var html = '<li>It ' + action + ' a ' +
              '<b>' + (fields.TakerPays.currency || 'STM') + '/' +
              (fields.TakerGets.currency || 'STM') + '</b> ' +
              '<type>Offer</type> ' +
              ...
              nodes.forEach(function (node) {
                switch (node.LedgerEntryType) {
                  ...
                  case 'Offer':
                    ...
                    break
                  case 'StoxumState':
                    html += renderStoxumState(type, node)
                    break
                  default:
                    html += '<li>It ' + type +
        ...
        function renderDescription(tx) {
          ...
          function renderOfferCreate() {
            var c1 = tx.tx.TakerPays.currency || 'STM'
            var c2 = tx.tx.TakerGets.currency || 'STM'
            ...
            if (tx.tx.Expiration) {
              var expiration = moment.unix(tx.tx.Expiration + STOXUM_EPOCH)
          ...
          function renderEscrowCreate() {
            ...
            if (tx.tx.CancelAfter) {
              html += '<br/>It can be cancelled after <date>' +
                moment((STOXUM_EPOCH + tx.tx.CancelAfter) * 1000).format('LLL') +
                '</date>.'
            }
            if (tx.tx.FinishAfter) {
              html += '<br/>It can be finished after <date>' +
                moment((STOXUM_EPOCH + tx.tx.FinishAfter) * 1000).format('LLL') +
                '</date>.'
            }
            return html
          }
        ...
        function displayTx() {
          ...
          div.selectAll('account').each(function () {
            ...
            stoxumName(account, function (name) {
              if (name) {
                el.html('<t>~</t><name>' +
