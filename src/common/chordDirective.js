angular.module('chordDiagram', []).directive('chordDiagram', ['$window', 'matrixFactory', 'stoxumName',

  function ($window, matrixFactory, stoxumName) {
    ...
    var link = function ($scope, $el, $attr) {
      ...
      var currencyOrder = ['XAU', 'XAG', 'BTC', 'LTC', 'STM', 'EUR', 'USD', 'GBP', 'AUD', 'NZD', 'CAD', 'CHF', 'JPY', 'CNY'];
      var stmRates = {
        STM: 1
      };
      var filters = {};
      ...
      $scope.setNormalizationRates = function (rates) {
        ...
        function setExchangeCurrency(currency) {
          ...
          html = exch.currency() === 'STM' ?
            '' : exch.rate().toPrecision(4) + ' <small>STM/' + exch.currency() + '</small>';
      ...
      $scope.drawChords = function (data) {
        ...
        chordData.forEach(function (chord) {
          stmRates[chord.base] = chord.base_rate;
        });
      ...
      function updateSidebars(d) {
        ...
        if (d && d.source) {
          ...
          //rate could be 0
          group.amount = stmRates[group.currency] ?
            group.volume / stmRates[group.currency] : 0
          ...
          for (var key in temp) {
            //rate could be 0
            temp[key].amount = stmRates[key] ?
              temp[key].volume / stmRates[key] : 0;
        ...
        div.selectAll('.issuer:not(.checked)').each(function (d) {
          ...
          if (issuer) {
            stoxumName(issuer, function (name) {
              el.text(name || issuer)
