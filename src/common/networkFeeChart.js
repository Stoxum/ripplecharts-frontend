var NetworkFees = function (options) {
  ...
  var chart = new LineChart({
    id: options.id + 'Chart',
    leftTitle: 'Total Fees (STM)',
    rightTitle: 'Average Fee (STM)',
    resize: true,
    minDomain: 0,
    tooltip: function (d, interval) {
      ...
      html += '<div><span>Average Fee:</span>' + d.y + ' <small>STM</small></div>' +
        '<div><span>Total Fees:</span>' + d.y2 + ' <small>STM</small></div>';
      return html;
    }
  });
